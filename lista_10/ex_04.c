...
void createNegativeImage(char* input_filename, char* output_filename) {
    ...
    for (int y = 0; y < header.height; y++) {
        ...
        for (int x = 0; x < header.width * (header.bpp / 8); x++) {
            row_buffer[x] = 255 - row_buffer[x];
        }
    ...
    printf("Negativo da imagem concluído. Imagem salva em %s.\n", output_filename);
}
...
int main() {
    ...
    createNegativeImage(input_filename, output_filename);
